...
// crear → agrega al instante y genera layout
async function create() {
  ...
  if (created?._id) {
    ...
    // si quieres abrir el layout inmediatamente:
...
<template>
  <section class="space-y-4">
    <header class="flex items-end justify-between">
      <div>
        ...
        <p class="text-sm text-neutral-500">
          Crea funciones y genera el layout de sillas.
        </p>
      </div>
      <NuxtLink
        to="/admin/movies"
        class="text-sm rounded-lg border border-theme px-3 py-2"
        >← Volver</NuxtLink
      >
    </header>
    ...
    <div
      v-if="!movieId"
      class="rounded-xl border border-red-500/40 bg-red-500/10 p-4 text-red-400"
    >
      Falta <b>movieId</b> en la URL. Entra desde <code>/admin/movies</code> →
      “Funciones”.
    </div>
    ...
    <template v-else>
      <form
        ...
        class="rounded-2xl border border-theme p-4 grid md:grid-cols-4 gap-3 bg-surface"
      >
        <input
      ...
      <div v-else class="grid gap-3">
        ...
        <div v-if="(list?.length || 0) === 0" class="text-neutral-500">
          Sin funciones aún.
        </div>
      </div>
    </template>
